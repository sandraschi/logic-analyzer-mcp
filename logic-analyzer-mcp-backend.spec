...
# PyInstaller spec — produce logic-analyzer-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller logic-analyzer-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/logic_analyzer_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/logic_analyzer_mcp", "logic_analyzer_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
...
exe = EXE(
    ...
    a.datas,
    [],
    name="logic-analyzer-mcp-backend",
    debug=False,
